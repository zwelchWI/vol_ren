...
//	buildEggRobot(&robot,&Crobot);
	...
	grid->init(Vector(-4,-4,-4),600,600,600,Vector(4,4,4),0.0);
	printf("GRID ALLOCATED\n");
	//BaseGrid<Color> * cgrid = new SparseGrid<Color>();
	//cgrid->init(Vector(-10,-8,-3),200,140,200,Vector(10,6,17),Color());
//	cgrid->init(Vector(-2,-2,-2),1000,1000,1000,Vector(2,2,2),Color());
//	printf("CGRID ALLOCATED\n");
	
	Particle part;
	part.nbWisps() = 1000000;
	part.pscale() = 2;
	part.wispRoughness() = .9;
	part.wispOctaves()   = clf.find( "-octaves", 1.0f, "Octaves of wisp" ); 
	part.wispFjump()     = clf.find( "-fjump", 2.0f, "Gamma of wisp" );
	part.wispFreq()      = clf.find( "-freq", 1.0f, "Frequency of wisp" );
	part.wispGamma() = clf.find( "-wispGamma",1.0f/3.0f, "Gamma of wisp" );	
	Wisp wisp(&part);
...
//	grid->sample(new Pyroclast(1));
	...
	keyDgrid->init(Vector(-4,-4,-4),200,200,200,Vector(4,4,4),0.0);
	...
	fillDgrid->init(Vector(-4,-4,-4),200,200,200,Vector(4,4,4),0.0);
	DSM *fill = new DSM(fillDgrid,Ps,ds);
	fill->sample(new Light(Vector(0,-6,0),Color(1,1,1,1)));
	DSMs.add(fill);
