...
int main(int argc, char** argv){
  ...
  //  float k = clf.find("-k",1.0f,"Scattering Coefficient");
	...
	BaseGrid<float> *grid1 = new FullGrid<float>();
	grid1->init(Vector(-3,-3,-3),300,300,300,Vector(3,3,3),0.0);	
	
	if(readGrid(grid1,"raw/bunny2layer.raw") == 0){
	...
	keyDgrid->init(Vector(-4,-4,-4),100,100,100,Vector(4,4,4),0.0);
	...
	fillDgrid->init(Vector(-4,-4,-4),100,100,100,Vector(4,4,4),0.0);
