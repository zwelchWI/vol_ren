...
//	buildEggRobot(&robot,&Crobot);
	...
	BaseGrid<float> *grid = new FullGrid<float>();
	//grid->init(Vector(-10,-8,-3),200,140,200,Vector(10,6,17),0.0);
	grid->init(Vector(-1,-1,-1),100,100,100,Vector(1,1,1),0.0);
	printf("GRID ALLOCATED\n");
	//BaseGrid<Color> * cgrid = new SparseGrid<Color>();
	//cgrid->init(Vector(-10,-8,-3),200,140,200,Vector(10,6,17),Color());
	//cgrid->init(Vector(-5,-5,-5),250,250,250,Vector(5,5,5),Color());
	//printf("CGRID ALLOCATED\n");
	
	Volume<float> *sdf = new SDF("models/cleanbunny.obj");
	printf("here %f\n",sdf->eval(Vector(-0.619318, 0.625586, 0.215304)));
//0.058072 0.680331 -0.674064
//	exit(0);
	//sdf = new ClampVolume(sdf,0,1);
	//printf("here2 %f\n",sdf->eval(Vector(1.607534, -1.248188, 0.32369)));

	
	
	if(readGrid(grid,"raw/cleanbunny100.raw") == 0){
		grid->sample(sdf);
		writeGrid(grid,"raw/cleanbunny100.raw");
	}
	
	//0.000000 1.780000 -1.640000 1.816040 1.816040
	Ps = new GridVolume(grid);
	//printf("here3 %f\n",Ps->eval(Vector(1.607534, -1.248188, 0.32369)));
	//exit(0);
//	Ps = new UnionVolume(Ps,new ImplicitSphere(1.474154));
//	Ps = new MaskVolume(Ps);
//	printf("here3 %f\n",Ps->eval(Vector(-0.980000, 1.080000, -1.800000)));
	
	//printf("%f %f %f\n",grid->getVect(6,26,99)[0],grid->getVect(6,26,99)[1],grid->getVect(6,26,99)[2]);
//	exit(0);

	Ps = new ClampVolume(Ps,0,1);
	Pe = new emptyField<float>(0.0);
	
	Cs = new SolidColorField(Color(.5,.5,.5,1));//CGridVolume(cgrid);
	Ce = new emptyField<Color>(Color(0.0,0.0,0.0,0.0));
	Volume<float> *k = new kField(75);
	...
	fill->sample(new Light(Vector(0,-5,0),Color(1,1,1,1)));
	DSMs.add(fill);
