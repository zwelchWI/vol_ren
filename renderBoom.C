...
int main(int argc, char** argv){
  ...
  //  float k = clf.find("-k",1.0f,"Scattering Coefficient");
	...
	Cs = new UnionColor(Cs,new ShapedColorField(Ps,Color(0.9,.9,0.9,1)));

	Ps = new UnionVolume(Ps,eggy);
	Volume<float> *krobot = new kField(15.0,Ps);
	Volume<float> *k = new UnionVolume(kpyro,krobot);
	Ps = new UnionVolume(Ps,pyro);
	...
	PyroTrail ptrail(ppts,310,370,frame,pPart,50);
	...
	Volume<float> *kPe = new kField(15.0,Pe);
	k = new UnionVolume(k,kPe);
