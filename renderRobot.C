...
//	bunny = new SDF("models/cleanbunny.obj");
	...
	bunnyGrid->init(Vector(-2,-3,-6),300,300,300,Vector(2,1,-2),0.0);
	...
	float eye = clf.find( "-eye", 0.0f, "red of eyes" );
	//Ps = new emptyField<float>(0.0);//new UnionVolume(eggy,bunny);
	Ps = new MaskVolume(Ps);//,0,1);//,0,1);
//	Ps = new UnionVolume(Ps,eggy);
	Volume<float> *leye = new ImplicitSphere(0.075 * eye);
	leye = new TranslateScalarField(leye,Vector(3.25,-.5,-.65));
	Volume<float> *reye = new ImplicitSphere(0.075 * eye);
	reye = new TranslateScalarField(reye,Vector(3.25,-.5,-.9));
*/
	Pe = new emptyField<float>(0.0);
	//Pe = new UnionVolume(leye,reye);;
	
	Pe = new MaskVolume(Pe);//,0,1);
	float roboC = clf.find( "-roboC", 0.0f, "robot color" );
	Cs = new MixColors(eggColor,new SolidColorField(Color(0.9,.9,0.9,1)),roboC);//CGridVolume(cgrid);
