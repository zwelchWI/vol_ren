...
int main(int argc, char** argv){
  ...
  //  float k = clf.find("-k",1.0f,"Scattering Coefficient");
	...
	camera.setNearPlane(15);
    camera.setFarPlane(25);
    Vector X;
...
//	buildEggRobot(&robot,&Crobot);
	...
	grid->init(Vector(-6,-6,-6),1000,1000,1000,Vector(6,6,6),0.0);
...
//	printf("CGRID ALLOCATED\n");
	...
	part.roughness()   = 0.9;
	part.pscale() = 5;
	part.octaves()   = clf.find( "-octaves", 1.0f, "Octaves of Stamped" ); 
	part.fjump()     = clf.find( "-fjump", 2.0f, "Gamma of Stamped" );
	part.freq()      = clf.find( "-freq", 1.0f, "Frequency of Stamped" );
	part.fade() = clf.find( "-fade",0.0f, "Fade of Stamped" );
...
//	grid->sample(new Pyroclast(1));
	...
	keyDgrid->init(Vector(-6,-6,-6),200,200,200,Vector(6,6,6),0.0);
	...
	fillDgrid->init(Vector(-6,-6,-6),200,200,200,Vector(6,6,6),0.0);
