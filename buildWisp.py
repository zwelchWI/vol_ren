	...
	for octave in octaves:
		for fjump in fjumps:
			for freq in freqs:
				...
				if frame < 10:
					padframe = "0" + padframe
				command = pbsGen + " -script " + executable + " -frame " + str(frame) + " -job " +str(jobLabel) + " -execName "+execName + " -pbsName "+pbsName + " -extras" + " -NX 960 -NY 540 -fname " + picDir + jobLabel +"/"+jobLabel +"."+ str(padframe) + "." + extension + " -ds " + str(ds) + " -octaves " + str(octave) + " -fjump " + str(fjump) + " -freq " + str(freq)+ " -wispGamma " + str(gamma)  	
				print command
				frame += 1
				os.system(command)
